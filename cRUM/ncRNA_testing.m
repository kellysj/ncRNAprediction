...
[tX,tT,tH] = largeFeat_haca_test_all_train();
[hX,hT,hH] = largeFeat_haca_test_all_hold();
...
%profile viewer
%toc;
save ripley_crum model
y = cRumClassify(model, trainset);
errs = sum(y(train_t == 0) > 0.5) + sum(y(train_t == 1) <= 0.5);
fprintf('M: %d\n', M);
fprintf('CRUM CLASSIFICATION train error: %.2f%%\n', errs / N *100);

% Test
y = cRumClassify(model, testset);
errs = sum(y(test_t == 0) > 0.5) + sum(y(test_t == 1) <= 0.5);
